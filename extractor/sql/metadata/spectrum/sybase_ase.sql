...
from (
   ...
   WHERE 1=1
   AND so.name = replace(replace(upper('${INPUT_TABLE_NAME}'),'[',''),']','')
   AND u.name = replace(replace(upper('${INPUT_TABLE_SCHEMA}'),'[',''),']','')
   AND c.name = replace(replace(upper('${PARTITION_FIELD}'),'[',''),']','')
   ...
   UNION ALL
    ...
    FROM
          sysobjects o
           ...
           ON o.id = i.id
    WHERE 1=1
    AND o.name = replace(replace(upper('${INPUT_TABLE_NAME}'),'[',''),']','')
    AND u.name = replace(replace(upper('${INPUT_TABLE_SCHEMA}'),'[',''),']','')
