...
from (
   ...
   UNION ALL
    ...
    CASE
     ...
     WHEN t.name in ('varbinary','image','varchar','char','nvarchar','nchar','text') then '['+c.name+']' + ' AS ' + case when c.name like '%/%' then right(str_replace(c.name,'/','_'),len(c.name)-1)else c.name end
    ...
    CASE
     ...
     WHEN t.name in ('varbinary','image','varchar','char','nvarchar','nchar','text') then '[' + c.name + ']' 
     ELSE '['+c.name+']'
    END AS casting,
    CASE 
       ...
       WHEN t.name in ('image','varbinary','varchar','char','nvarchar','nchar')then 'varchar('+cast(cast((c.length*0.3)+c.length as bigint) as varchar)+')'
    ...
    ( '{"name": "' + lower(case when c.name like '%/%' then right(str_replace(c.name,'/','_'),len(c.name)-1)else c.name end) + '','type":' + 
     CASE
       ...
       WHEN t.name in ('image','varbinary','varchar','char','nvarchar','nchar','text','date','time','datetime')then '["null", "string"]'
